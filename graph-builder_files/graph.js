...
function Vertex(id, value, x, y){
	...
	this._draw    = function(cx, style, label){
		...
		cx.fill()
		cx.stroke()
		drawLabel(this.id, this.x, this.y - 14) //show ID
		if(label && (typeof this.value == s || typeof this.value == n) && this.value !== ""){ //id there is a label
...
function drawLabel(label, x, y){
	if((typeof label == s || typeof label == n) && label !== ""){
		...
		cx.beginPath()
		cx.arc(x, y, labelStyle.vertexradius*1.7, 0, 2*Math.PI, true)
		cx.closePath()
		cx.fill()
		cx.stroke()
		cx.fillStyle = "#000"
...
$(document).ready(function(){
  $(document).keypress(function(e){
    if(e.charCode == 103){
          $('#btnaddvertex').trigger('click');
    }
    if(e.charCode == 101){
          $('#btnaddedge').trigger('click');
    }
    if(e.charCode == 102){
          $('#info .removebtn').trigger('click');
    }
  })
